...
export function renderCorregirExercise(exercise, container, isRetry = false) {
  ...
  container.innerHTML = `
    <div class="corregir-card">
      ...
      <div class="corregir-eyebrow">🔍 Corrige la frase</div>
      <div class="error-highlight">${fraseConError}</div>
    ...
    <div class="modal-friend">
      ...
      <div class="comparison-text-block">
        <div class="comparison-row">
          ...
          <div class="comparison-row-text error-highlight">${fraseConError}</div>
        </div>
        <div class="comparison-row">
          <span class="comparison-row-label">✅ Frase correcta</span>
          <div class="comparison-row-text correct-highlight">${fraseCorrecta}</div>
        </div>
        <div class="comparison-row">
          <span class="comparison-row-label">✏️ Tu respuesta</span>
          <div class="comparison-row-text ${isCorrect ? 'correct-highlight' : ''}" style="${isCorrect ? '' : 'color:#fbbf24;'}">${window._escHTML(userAnswer)}</div>
        </div>
        <div class="comparison-row">
